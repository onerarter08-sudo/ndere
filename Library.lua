...
-- Тот самый фрейм ЗА всеми групбоксами (с отступом под Топ-бар) --тягай
    local TabContainer = Library:Create('Frame', {
        ...
        Size = UDim2.new(1, -170, 1, -60), -- Уменьшили высоту, чтобы снизу остался аккуратный отступ 12px
        ZIndex = 2;
        Parent = Outer;
    });
...
local LeftSide = Library:Create('ScrollingFrame', {
            BackgroundTransparency = 1;
            BorderSizePixel = 0;
            Position = UDim2.new(0, 5, 0, 10); -- БЫЛО 35, СТАЛО 10 (подтянули наверх!)
            Size = UDim2.new(0.5, -12, 1, -20); -- Увеличили размер, чтобы они доставали до низа
            CanvasSize = UDim2.new(0, 0, 0, 0);
